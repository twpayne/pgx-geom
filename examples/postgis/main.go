...
var (
	connStr = flag.String("conn", "database=pgxgeomtest", "connection string")
...
func (w *Waypoint) MarshalJSON() ([]byte, error) {
	geometry, err := geojson.Marshal(w.Geometry)
	if err != nil {
		return nil, err
	}
...
func (w *Waypoint) UnmarshalJSON(data []byte) error {
	...
	var geom geom.T
	if err := geojson.Unmarshal(geoJSONWaypoint.Geometry, &geom); err != nil {
		return err
	}
...
func populateDB(ctx context.Context, conn *pgx.Conn) error {
	...
	for _, waypoint := range []Waypoint{
		{
			Name:     "London",
			Geometry: geom.NewPoint(geom.XY).MustSetCoords(geom.Coord{0.1275, 51.50722}).SetSRID(4326),
		},
		{
			Name:     "Berlin",
			Geometry: geom.NewPoint(geom.XY).MustSetCoords(geom.Coord{13.405, 52.52}).SetSRID(4326),
		},
	} {
