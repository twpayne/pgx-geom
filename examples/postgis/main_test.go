...
func TestIntegration(t *testing.T) {
	...
	var (
		database = "pgxgeomdatabase"
		user     = "pgxgeomuser"
		password = "pgxgeompassword"
	)

	pgContainer, err := postgres.Run(ctx, "docker.io/postgis/postgis:16-3.4",
	...
	assert.NoError(t, err)
	assert.NoError(t, pgxgeom.Register(ctx, conn))
